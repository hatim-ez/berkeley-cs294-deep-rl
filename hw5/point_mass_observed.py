...
class ObservedPointEnv(Env):
    ...
    def reset_task(self, is_evaluation=False):
        ...
        self.one_hot_task = [0, 0, 0, 0]
        self.one_hot_task[self.task_idx] = 1

    def reset(self):
        # self._state = np.concatenate((np.array([0, 0], dtype=np.float32), self.one_hot_task), axis=0)
        self._state = np.array([0, 0], dtype=np.float32)
        return self._get_obs()

    def _get_obs(self):
        return np.concatenate((self._state, self.one_hot_task), axis=0)
        #return np.copy(self._state)

    def step(self, action):
        #print("state before action: ", self._state)
        x, y = self._state.flatten()
        #action = np.concatenate((action, np.zeros(len(self.tasks))), axis=0)
        #x, y = self._state[0:2]
        #x, y = self._state[0:2]
        ...
        #print("state after action: ", self._state)
        #print("observation copy of state before action: ", ob)
