...
class QLearner(object):
  ...
  def __init__(
    ...
    self.mean_episode_reward_log = []
    self.best_mean_episode_reward_log = []
    self.episodes_log = []

    self.q_func_source = q_func(obs_t_float, self.num_actions, scope="q_func", reuse=False)

    self.q_func_target = q_func(obs_tp1_float, self.num_actions, scope="q_func_target", reuse=False)

    if double_q:
        #q_func_target_prime = q_func(obs_tp1_float, self.num_actions, scope="q_func_target_prime", reuse=tf.AUTO_REUSE)
        y = tf.stop_gradient(self.rew_t_ph + gamma * tf.multiply(1.0 - self.done_mask_ph, tf.reduce_max(self.q_func_target, axis=1)))

    else:
        y = self.rew_t_ph + gamma * tf.reduce_max(self.q_func_target, reduction_indices=1)

    act_one_hot = tf.one_hot(self.act_t_ph, depth=self.num_actions, dtype=tf.float32)

    diff = y - tf.reduce_sum(act_one_hot*self.q_func_source, axis=1)

    self.total_error = huber_loss(diff, delta=1)

    q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='q_func')
    target_q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='q_func_target')
    #
    # self.q_func_target = tf.stop_gradient(q_func(obs_tp1_float, self.num_actions, scope="q_func_target", reuse=False))
    # q_func_target_prime = q_func(obs_tp1_float, self.num_actions, scope="q_func_target_prime", reuse=tf.AUTO_REUSE)
    #
    # if double_q:
    #     act_one_hot_prime = tf.one_hot(tf.argmax(self.q_func_source, axis=1), depth=self.num_actions, dtype=tf.float32)
    #     y = self.rew_t_ph + gamma * tf.reduce_sum(act_one_hot_prime * self.q_func_target, axis=1)
    #
    # else:
    #     y = self.rew_t_ph + gamma * tf.reduce_max(self.q_func_target, reduction_indices=1)
    #
  ...
  def step_env(self):
    ...
    if not self.model_initialized or random.random() < epsilon:
        ...
        q_vals = self.session.run(self.q_func_source, {self.obs_t_ph: input_batch[None, :]})
        act = np.argmax(q_vals)

    last_obs, rew, done, info = self.env.step(act)
  ...
  def log_progress(self):
    ...
    if self.t % self.log_every_n_steps == 0 and self.model_initialized:
      ...
      self.time_step_log.append(self.t)
      self.mean_episode_reward_log.append(self.mean_episode_reward)
      self.best_mean_episode_reward_log.append(self.best_mean_episode_reward)
      self.exploration_log.append(self.exploration)
      self.learning_rate_log.append(self.optimizer_spec.lr_schedule.value(self.t))
      self.episodes_log.append(len(episode_rewards))

      self.log = ({'time_step_log': self.time_step_log,
                 'mean_episode_reward_log': self.mean_episode_reward_log,
                 'best_mean_episode_reward_log': self.best_mean_episode_reward_log,
                 'learning_rate_log': self.learning_rate_log,
                 'episodes_log': self.episodes_log,
...
def learn(*args, **kwargs):
  ...
  while not alg.stopping_criterion_met():
    ...
    with open("data/{0}.pkl".format(alg.rew_file[:-4]), 'wb') as f:
        pickle.dump(logs, f, protocol=3)
