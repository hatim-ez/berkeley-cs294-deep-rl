...
class ModelBasedRL(object):
    ...
    def _train_policy(self, dataset):
        ...
        for i in range(self._training_epochs):
            for r_num, (states, actions, next_states, _, _) in enumerate(dataset.random_iterator(self._training_batch_size)):
                ...
                loss = self._policy.train_step(states=states, actions=actions, next_states=next_states)
                losses.append(loss)
    ...
    def run_q1(self):
        ...
        for r_num, (states, actions, _, _, _) in enumerate(self._random_dataset.rollout_iterator()):
            ...
            for i in range(1, len(states)):
                next_state_pred = self._policy.predict(state=states[i-1], action=actions[i-1])
            ...
            for i, (ax, state_i, pred_state_i) in enumerate(zip(axes.ravel(), states.T, pred_states.T)):
                ...
                ax.plot(pred_state_i, color='r') #, linestyle='dashed')
            plt.tight_layout()
    ...
    def run_q2(self):
        ...
        ### PROBLEM 2
        ### YOUR CODE HERE
        logger.info('Gathering policy dataset')
